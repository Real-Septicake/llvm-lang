#include "arg_parse.hpp"
#include "ast_printer.hpp"
#include "compiler.hpp"
#include "error.hpp"
#include "parser.hpp"
#include "scanner.hpp"
#include "stmt.hpp"

#include <fstream>
#include <iostream>
#include <termcolor/termcolor.hpp>
#include <vector>

int main(int argc, char *argv[]) {
    parse_args(argc, argv);
    if (get_args().files.empty()) {
        std::cerr << termcolor::bright_red << termcolor::bold
                  << "oi, give me a file you daft cu-" << termcolor::reset
                  << std::endl
                  << "fatal error: no input file." << std::endl;
        return 1;
    }

    std::ifstream file(get_args().files[0], std::ios::binary);
    std::string source((std::istreambuf_iterator<char>{file}), {});
    if (source.length() == 0) {
        std::cerr << termcolor::bright_red << termcolor::bold
                  << "So no code? *breaks skateboard*" << termcolor::reset
                  << std::endl;
        return 0;
    }
    Scanner scanner(source);
    std::vector<Token *> toks = scanner.scanTokens();

    if (get_args().debug) {
        std::cout << "\nTokens: " << std::endl;
        for (auto tok : toks) {
            printToken(*tok);
        }
    }

    parser::Parser parser(toks);
    std::vector<AST::Stmt *> stmts = parser.parse();
    if (error::errored)
        return 1;

    if (get_args().debug) {
        ASTPrinter printer;
        std::cout << "\nAST:" << std::endl;
        for (auto stmt : stmts) {
            if (stmt == nullptr) {
                std::cerr << "Error parsing file" << std::endl;
                return 1;
            }
            stmt->accept(&printer);
        }
    }

    std::string filename =
        std::string(get_args().files[0])
            .substr(std::string(get_args().files[0]).find_last_of("/\\") + 1);
    filename                 = filename.substr(0, filename.find_last_of("."));
    compiler::Compiler *comp = new compiler::Compiler(filename);
    for (auto stmt : stmts) {
        stmt->codegen(comp);
    }
    if (comp->errored) {
        return 1;
    }

    if (get_args().debug) {
        comp->print_code();
    }

    comp->verify();
    if (comp->errored) {
        return 1;
    }
    if (comp->write(get_args().out_file)) {
        return 1;
    }
}
